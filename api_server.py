...
# Global storage for tasks (in-memory for simplicity)
tasks_storage = {}
...
def real_video_processing(video_id, video_path):
    ...
    try:
        ...
        tasks_storage[video_id]["updated_at"] = datetime.datetime.now().isoformat()
        tasks_storage[video_id]["pdf_path"] = str(pdf_path)
...
def get_video_status(video_id):
    """Get REAL video processing status - no authentication required"""
    try:
        task = tasks_storage.get(video_id)
        if not task:
        ...
        # Add PDF path if completed
        if task.get("status") == "completed" and "pdf_path" in task:
            status_response["pdf_path"] = task["pdf_path"]
